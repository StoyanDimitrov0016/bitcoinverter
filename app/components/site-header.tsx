...
const NAVIGATION_ITEMS = [
  ...
  { href: "#percentile", labelKey: "percentile" },
...
export function SiteHeader({ price }: SiteHeaderProps) {
  ...
  const tSiteHeader = useTranslations("SiteHeader");
      ...
      <div className="layout-container flex flex-wrap items-center gap-x-1 gap-y-3 py-3 sm:gap-x-4">
        ...
        >
          {NAVIGATION_ITEMS.map((item) => (
            <HeroLink key={item.href} className="shrink-0 text-muted" href={item.href}>
              {t(item.labelKey)}
            </HeroLink>
          ))}
        </nav>
        ...
        <div className="ms-auto flex shrink-0 items-center gap-1 sm:gap-2">
          ...
          <ThemeToggle />
          <HeroLink
            aria-label={tSiteHeader("githubAriaLabel")}
...
function LivePriceTicker({ prices }: LivePriceTickerProps) {
  const t = useTranslations("SiteHeader");
  const eur = formatFiat(prices.EUR, "EUR");
  const usd = formatFiat(prices.USD, "USD");

  return (
    <Chip className="w-24 justify-center bg-surface px-0.5 text-accent" size="sm">
      <output aria-atomic="true" className="sr-only">
        {t("livePricesAnnouncement", { eur, usd })}
      </output>
      <span aria-hidden="true" className="h-5 overflow-hidden font-mono font-medium">
        <span className="price-ticker-track flex flex-col">
          <span className="flex h-5 items-center justify-center whitespace-nowrap">{eur}</span>
          <span className="flex h-5 items-center justify-center whitespace-nowrap">{usd}</span>
          <span className="flex h-5 items-center justify-center whitespace-nowrap">{eur}</span>
        </span>
      </span>
    </Chip>
